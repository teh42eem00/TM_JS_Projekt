    ...
    def __init__(self, machine_items, machine_coins_input, machine_coins_change):
        ...
        for coin in logic.possible_coins:
            ttk.Button(self._mainframe, text="Wrzuć " + str(coin) + "zł",
                       command=lambda lcoin=coin: self.action_on_money(lcoin)).grid(column=4, row=self._coin_row)  # lambda
            self._coin_row += 1
        [ttk.Button(self._mainframe, text=str(digit + 1),
         ...
         range(9)]  # lambda
        ttk.Button(self._mainframe, text="0", command=lambda: self.action_on_choice(0)).grid(row=5, column=1)  # lambda
        ...
        ttk.Button(self._mainframe, text="Monety - Reszta",
                   command=lambda: print(self._machine_coins_change.return_array_of_value())).grid(column=0,
                                                                                                   row=6)  # lambda
        ttk.Button(self._mainframe, text="Monety - Wplacone",
                   command=lambda: print(self._machine_coins_input.return_array_of_value())).grid(column=0,
                                                                                                  row=7)  # lambda
